...
fn test_list_truncation() {
    ...
    assert!(output.contains("… (84 more)"), 
        "SafeDebug didn't properly truncate the list: {}", output);
}
...
fn test_string_truncation() {
    ...
    assert!(output.contains("..."), 
        "SafeDebug didn't properly truncate the string: {}", output);
    
    assert!(output.len() < long_string.len() + 5, 
        "SafeDebug didn't truncate the string: output length = {}, original length = {}", 
        output.len(), long_string.len());
}
...
fn test_object_truncation() {
    ...
    assert!(output.contains("… (14 more)"), 
        "SafeDebug didn't properly truncate the object: {}", output);
}
