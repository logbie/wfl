...
fn assert_typechecks(src: &str) {
    ...
    assert!(
        result.is_ok(),
        "Expected program to type-check successfully: {:?}",
        result.err()
    );
}
...
fn assert_type_error(src: &str, expected_error: &str) {
    ...
    assert!(
        result.is_err(),
        "Expected type error but program type-checked successfully"
    );
    let errors = result.err().unwrap();
    assert!(
        errors.iter().any(|e| e.message.contains(expected_error)),
