...
fn closure_cycle() {
    ...
    println!("Strong count before drop: {}", Rc::strong_count(&global_env));
...
fn parser_stability() {
    ...
    assert!(delta.abs() < 10_240, 
        "Memory usage grew by {} KB between parses, which exceeds the 10 MB limit", delta);
}
