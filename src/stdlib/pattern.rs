...
pub enum PatternPart {
    ...
    Letters { min: usize, max: Option<usize> },
    Whitespace { min: usize, max: Option<usize> },
    Placeholder(String),
    Optional(Box<PatternPart>),
    OneOrMore(Box<PatternPart>),
    Exactly { count: usize, part: Box<PatternPart> },
    Between { min: usize, max: usize, part: Box<PatternPart> },
    ...
    pub fn parse(pattern_str: &str) -> Result<Self, String> {
        ...
        while current_pos < pattern_str.len() {
            ...
            if remaining.starts_with("digit") {
                ...
                let is_plural = current_pos < pattern_str.len() && &pattern_str[current_pos..current_pos+1] == "s";
                if is_plural {
                    current_pos += 1;
                }
                ...
                if let Some((min, max, new_pos)) = parse_quantifier(pattern_str, current_pos) {
                    ...
                    regex_str.push_str(&format!("\\d{{{},{}}}", min, max.map_or("".to_string(), |m| m.to_string())));
                } else if is_plural {
                    parts.push(PatternPart::Digits { min: 1, max: None });
                    regex_str.push_str("\\d+");
                } else {
                    parts.push(PatternPart::Digits { min: 1, max: Some(1) });
                    regex_str.push_str("\\d");
                }
                continue;
            }
            ...
            if remaining.starts_with("letter") {
                ...
                if let Some((min, max, new_pos)) = parse_quantifier(pattern_str, current_pos) {
                    ...
                    regex_str.push_str(&format!("[a-zA-Z]{{{},{}}}", min, max.map_or("".to_string(), |m| m.to_string())));
                } else if is_plural {
                    parts.push(PatternPart::Letters { min: 1, max: None });
                    regex_str.push_str("[a-zA-Z]+");
                } else {
                    parts.push(PatternPart::Letters { min: 1, max: Some(1) });
                    regex_str.push_str("[a-zA-Z]");
                }
                continue;
            }
            ...
            if remaining.starts_with("whitespace") {
                ...
                if let Some((min, max, new_pos)) = parse_quantifier(pattern_str, current_pos) {
                    ...
                    regex_str.push_str(&format!("\\s{{{},{}}}", min, max.map_or("".to_string(), |m| m.to_string())));
                } else {
            ...
            if remaining.starts_with("optional ") {
                ...
                while current_pos < pattern_str.len() && pattern_str[current_pos..current_pos+1].trim().is_empty() {
                    current_pos += 1;
                }
            ...
            if remaining.starts_with("exactly ") {
                ...
                while current_pos < pattern_str.len() && pattern_str[current_pos..current_pos+1].chars().next().unwrap().is_ascii_digit() {
                    current_pos += 1;
                }
                ...
                let count = pattern_str[num_start..current_pos].parse::<usize>()
                ...
                parts.push(PatternPart::Exactly { count, part: Box::new(part) });
            ...
            if remaining.starts_with("between ") {
                ...
                let min = pattern_str[num_start..current_pos].parse::<usize>()
                ...
                let max = pattern_str[num_start..current_pos].parse::<usize>()
                ...
                parts.push(PatternPart::Between { min, max, part: Box::new(part) });
            ...
            let c = pattern_str[current_pos..current_pos+1].chars().next().unwrap();
...
fn parse_quantifier(pattern_str: &str, pos: usize) -> Option<(usize, Option<usize>, usize)> {
    ...
    if current_pos + 7 <= pattern_str.len() && &pattern_str[current_pos..current_pos+7] == "or more" {
        current_pos += 7;
...
fn parse_part(pattern_str: &str, pos: usize) -> Result<(PatternPart, String, usize), String> {
    ...
    if remaining.starts_with("digit") {
        ...
        if let Some((min, max, new_pos)) = parse_quantifier(pattern_str, current_pos) {
            ...
            let regex = format!("\\d{{{},{}}}", min, max.map_or("".to_string(), |m| m.to_string()));
            ...
            let part = if is_plural {
                ...
                PatternPart::Digits { min: 1, max: Some(1) }
            };
    ...
    if remaining.starts_with("letter") {
        ...
        if let Some((min, max, new_pos)) = parse_quantifier(pattern_str, current_pos) {
            ...
            let regex = format!("[a-zA-Z]{{{},{}}}", min, max.map_or("".to_string(), |m| m.to_string()));
            ...
            let part = if is_plural {
                ...
                PatternPart::Letters { min: 1, max: Some(1) }
            };
    ...
    if remaining.starts_with("whitespace") {
        ...
        if let Some((min, max, new_pos)) = parse_quantifier(pattern_str, current_pos) {
            ...
            let regex = format!("\\s{{{},{}}}", min, max.map_or("".to_string(), |m| m.to_string()));
...
pub fn native_pattern_split(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match Pattern::parse(&pattern_str) {
        Ok(pattern) => {
            ...
            let values: Vec<Value> = parts.into_iter()
...
pub fn register(env: &mut Environment) {
    env.define("matches_pattern", Value::NativeFunction(native_pattern_matches));
    env.define("find_pattern", Value::NativeFunction(native_pattern_find));
    env.define("replace_pattern", Value::NativeFunction(native_pattern_replace));
    env.define("split_by_pattern", Value::NativeFunction(native_pattern_split));
}
