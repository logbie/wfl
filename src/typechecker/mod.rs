    ...
    fn check_statement_types(&mut self, statement: &Statement) {
        match statement {
            ...
            Statement::OpenFileStatement {
                ...
                if let Some(symbol) = self.analyzer.get_symbol_mut(variable_name) {
                    symbol.symbol_type = Some(Type::Custom(crate::common::ident::intern("File")));
                }
            }
            Statement::ReadFileStatement {
                ...
                if file_type != Type::Custom(crate::common::ident::intern("File"))
                    && file_type != Type::Unknown
                    && file_type != Type::Error
                {
                    self.type_error(
                        "Expected a File object".to_string(),
                        Some(Type::Custom(crate::common::ident::intern("File"))),
                        Some(file_type),
                        *line,
                        *column,
                    );
                ...
                match object_type {
                    ...
                    _ => {
                        self.type_error(
                            ...
                            Some(Type::Custom(crate::common::ident::intern("Object"))),
                            Some(object_type),
                            *line,
                            *column,
                        );
