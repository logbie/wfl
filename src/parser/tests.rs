...
fn test_parse_if_statement() {
    ...
    if let Ok(Statement::IfStatement { condition, then_block, else_block }) = result {
...
fn test_parse_expression() {
    ...
    if let Ok(Expression::BinaryOperation { left, operator, right }) = result {
        ...
        if let Expression::BinaryOperation { left, operator, right } = *right {
            if let Expression::Literal(Literal::Integer(n)) = *left {
                assert_eq!(n, 3);
            } else {
                panic!("Expected integer literal");
