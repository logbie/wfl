...
impl Linter {
    ...
    pub fn lint(&self, program: &Program, source: &str, file_path: &str) -> (Vec<WflDiagnostic>, bool) {
        ...
        (all_diagnostics, all_diagnostics.is_empty())
    }
    ...
    pub fn load_config(&mut self, dir: &Path) {
        if let Ok(config) = crate::config::load_config(dir) {
            if let Some(max_line_length) = config.max_line_length {
                self.set_max_line_length(max_line_length);
            }
            if let Some(max_nesting_depth) = config.max_nesting_depth {
                self.set_max_nesting_depth(max_nesting_depth);
            }
        }
    }
}
...
impl LintRule for LineLengthRule {
    ...
    fn apply(&self, program: &Program, reporter: &mut DiagnosticReporter, file_id: usize) -> Vec<WflDiagnostic> {
        ...
        let max_length = 100; // Default, should be configurable
...
impl LintRule for NestingDepthRule {
    ...
    fn apply(&self, program: &Program, reporter: &mut DiagnosticReporter, file_id: usize) -> Vec<WflDiagnostic> {
        ...
        let max_depth = 5; // Default, should be configurable
