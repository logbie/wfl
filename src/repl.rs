...
use crate::parser::{Parser, ast::{Program, Statement}};
...
impl ReplState {
    ...
    fn is_input_incomplete(&self, tokens: &[TokenWithPosition]) -> bool {
        ...
        match parser.parse() {
            Err(errors) => {
                errors.iter().any(|e| e.message.contains("Unexpected end of input") || 
        ...
        let program = match parser.parse() {
            ...
            Err(errors) => {
                let error_messages: Vec<String> = errors.iter()
                    .map(|e| format!("Parse error at line {}, column {}: {}", e.line, e.column, e.message))
                    .collect();
...
pub fn run_repl() -> RustylineResult<()> {
    ...
    loop {
        let prompt = if repl_state.in_multiline { "... " } else { "wfl> " };
