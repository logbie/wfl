...
impl Interpreter {
    ...
    fn execute_statement(&self, stmt: &Statement, env: Rc<RefCell<Environment>>) -> Result<Value, RuntimeError> {
        match stmt {
            ...
            Statement::ForEachLoop { item_name, collection, reversed, body, line, column } => {
    ...
    fn execute_block(&self, statements: &[Statement], env: Rc<RefCell<Environment>>) -> Result<Value, RuntimeError> {
        let mut last_value = Value::Null;
        ...
        match expr {
            ...
            Expression::Variable(name, line, column) => {
                if name == "count" {
                    if let Some(value) = env.borrow().get(name) {
                        ...
                        println!("Warning: Using 'count' outside of a count loop context at line {}, column {}", line, column);
                        Ok(Value::Number(0.0))
                    }
            ...
            Expression::BinaryOperation { left, operator, right, line, column } => {
                ...
                match operator {
                    ...
                    Operator::GreaterThanOrEqual => self.greater_than_equal(left_val, right_val, *line, *column),
            ...
            Expression::FunctionCall { function, arguments, line, column } => {
                ...
                match function_val {
                    ...
                    Value::NativeFunction(native_fn) => {
                        native_fn(arg_values).map_err(|e| {
    ...
    fn call_function(&self, func: &FunctionValue, args: Vec<Value>, line: usize, column: usize) -> Result<Value, RuntimeError> {
    ...
    fn divide(&self, left: Value, right: Value, line: usize, column: usize) -> Result<Value, RuntimeError> {
        match (left, right) {
            (Value::Number(a), Value::Number(b)) => {
                if b == 0.0 {
                    Err(RuntimeError::new("Division by zero".to_string(), line, column))
                } else {
                    Ok(Value::Number(a / b))
                }
            },
    ...
    fn greater_than(&self, left: Value, right: Value, line: usize, column: usize) -> Result<Value, RuntimeError> {
        match (left, right) {
            ...
            (a, b) => Err(RuntimeError::new(
                format!("Cannot compare {} and {} with >", a.type_name(), b.type_name()),
                line, column
            )),
        }
    }
    ...
    fn contains(&self, left: Value, right: Value, line: usize, column: usize) -> Result<Value, RuntimeError> {
        match (left, right) {
            ...
            (a, b) => Err(RuntimeError::new(
                format!("Cannot check if {} contains {}", a.type_name(), b.type_name()),
                line, column
            )),
        }
    }
